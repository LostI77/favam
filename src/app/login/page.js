...
export default function Login() {
        ...
        try {
            if (pathname === "/login") {
                ...
                const res = await fetch('/api/signup', {
                    method: 'POST',
                    body: JSON.stringify({ username, password, fullName, email, age, dni, birthDate}),
                    headers: {'Content-Type': 'application/json'}
                });
                if(!res.ok) {
                    throw new Error(`HTTP error! status: ${res.status}`);
    ...
    const signUpinputs = [
        ...
        { key: 'sign-up-input-4', type: 'number', labelName: 'Age:', name: 'new-user-age', placeholder: 'Ingresar tu edad', data: (e) => setAge(e.target.value), },
        ...
        { key: 'sign-up-input-6', type: 'number', labelName: 'Dni:', name: 'new-user-dni', placeholder: 'Ingresar numero DNI', data: (e) => setDni(e.target.value), },
